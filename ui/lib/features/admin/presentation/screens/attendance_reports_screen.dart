...
class _AttendanceReportsScreenState
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 400.h, child: _buildDataTable(true)),
                SizedBox(height: 24.h),
              ],
            ),
          ),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            SizedBox(
              height: 280.h,
              child: _buildPieChart(total, onTime, late, absent, false),
            ),
            SizedBox(height: 16.h),
  ...
  Widget _buildHeader() {
    return Column(
      ...
      mainAxisSize: MainAxisSize.min,
      children: [
        Text('Attendance Reports', style: AppTheme.headingMedium(context)),
        SizedBox(height: 4.h),
        Text(
  ...
  Widget _buildBarChart(bool isDesktop) {
    ...
    for (final record in _records) {
      ...
      dailyData[date]![record.status] =
          (dailyData[date]![record.status] ?? 0) + 1;
    }
  ...
  double _calculateMaxY(Map<DateTime, Map<String, int>> dailyData) {
    ...
    for (final data in dailyData.values) {
      final total =
          (data['On Time'] ?? 0) + (data['Late'] ?? 0) + (data['Absent'] ?? 0);
            ...
            child: _records.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) =>
                        _buildTableRow(_records[index], index, isDesktop),
                  ),
          ),
        ],
      ),
        ...
        children: [
          ...
          _records.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemCount: _records.length,
                  itemBuilder: (context, index) =>
                      _buildTableRow(_records[index], index, false),
                ),
        ],
      ),
    );
      ...
      child: Row(
        children: [
          Expanded(
            ...
            child: Text(
              record.employeeName,
              style: TextStyle(fontSize: isDesktop ? 12.sp : 10.sp),
              overflow: TextOverflow.ellipsis,
            ),
          ),
          Expanded(
  ...
  Future<void> _downloadExcel() async {
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename: 'attendance_report_${_formatFileDate()}.xlsx',
    );
  }
  ...
  Future<void> _downloadCSV() async {
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename: 'attendance_report_${_formatFileDate()}.csv',
    );
  }
  ...
  Future<void> _downloadPDF() async {
    ...
    await Printing.sharePdf(
      bytes: bytes,
      filename: 'attendance_report_${_formatFileDate()}.pdf',
    );
  }

  String _formatFileDate() =>
      '${_startDate.day}_${_startDate.month}_${_startDate.year}';
}
